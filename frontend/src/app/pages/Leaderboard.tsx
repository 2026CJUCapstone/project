...
function getTierBadgeClass(tier: string) {
  if (tier.startsWith('Master')) return 'border-pink-500/30 bg-pink-500/10 text-pink-700 dark:text-pink-300';
  if (tier.startsWith('Diamond')) return 'border-blue-500/30 bg-blue-500/10 text-blue-700 dark:text-blue-300';
  if (tier.startsWith('Platinum')) return 'border-cyan-500/30 bg-cyan-500/10 text-cyan-700 dark:text-cyan-300';
  if (tier.startsWith('Gold')) return 'border-yellow-500/30 bg-yellow-500/10 text-yellow-700 dark:text-yellow-300';
  if (tier.startsWith('Silver')) return 'border-slate-500/30 bg-slate-500/10 text-slate-700 dark:text-slate-300';
  if (tier.startsWith('Bronze')) return 'border-amber-700/30 bg-amber-700/10 text-amber-800 dark:text-amber-300';
  if (tier.startsWith('Iron')) return 'border-zinc-500/30 bg-zinc-500/10 text-zinc-700 dark:text-zinc-300';
  return 'border-gray-300 bg-gray-50 text-gray-500 dark:border-[#333] dark:bg-[#1a1a1a] dark:text-gray-400';
}
...
export function Leaderboard() {
  ...
  const totalRating = useMemo(
    () => entries.reduce((sum, entry) => sum + entry.rating, 0),
    [entries],
  );
  const totalSolved = useMemo(
    () => entries.reduce((sum, entry) => sum + entry.solvedCount, 0),
    [entries],
  );
        ...
        <div className="mx-auto flex max-w-6xl flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              어려운 문제와 꾸준한 해결 기록을 함께 반영한 B++ 레이팅 기준 순위입니다.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 dark:border-[#333] dark:bg-[#151515]">
              <p className="text-xs font-semibold text-gray-500 dark:text-gray-400">누적 레이팅</p>
              <p className="mt-1 text-lg font-bold text-blue-600 dark:text-blue-400">{totalRating.toLocaleString()}</p>
            </div>
            <div className="rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 dark:border-[#333] dark:bg-[#151515]">
              <p className="text-xs font-semibold text-gray-500 dark:text-gray-400">해결 수</p>
              <p className="mt-1 text-lg font-bold text-emerald-600 dark:text-emerald-400">{totalSolved.toLocaleString()}</p>
            </div>
            <button
        ...
        <div className="mx-auto flex max-w-6xl flex-col gap-5 pb-10">
          {topEntry && (
            <div className="grid gap-4 rounded-lg border border-yellow-500/30 bg-yellow-50 px-5 py-4 dark:bg-yellow-500/10 md:grid-cols-[1fr_auto] md:items-center">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    <p className="text-xl font-bold">{topEntry.username}</p>
                    <span className={`inline-flex rounded-md border px-2 py-0.5 text-xs font-semibold ${getTierBadgeClass(topEntry.tier)}`}>
                      {topEntry.tier}
                    </span>
                  </div>
                </div>
              ...
              <div className="text-left md:text-right">
                <p className="text-xs font-semibold text-gray-500 dark:text-gray-400">B++ Rating</p>
                <p className="mt-1 font-mono text-2xl font-bold text-blue-600 dark:text-blue-400">
                  {topEntry.rating.toLocaleString()}
                </p>
              </div>
            </div>
          )}
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[820px]">
              <div className="grid grid-cols-[72px_minmax(0,1fr)_128px_128px_96px_96px] border-b border-gray-200 px-4 py-3 text-xs font-semibold text-gray-500 dark:border-[#333] dark:text-gray-400">
                <div className="text-center">순위</div>
                <div>사용자</div>
                <div>티어</div>
                <div className="text-right">레이팅</div>
                <div className="text-right">해결</div>
              ...
              {!isLoading && !errorMessage && entries.map((entry) => (
                <div
                  ...
                  className={`grid grid-cols-[72px_minmax(0,1fr)_128px_128px_96px_96px] items-center rounded-lg border px-4 py-3 transition-colors ${
                    entry.rank <= 3
                ...
                >
                  ...
                  <div>
                    <span className={`inline-flex rounded-md border px-2 py-0.5 text-xs font-semibold ${getTierBadgeClass(entry.tier)}`}>
                      {entry.tier}
                    </span>
                  </div>
                  <div className="flex items-center justify-end gap-1 text-right font-mono font-bold text-blue-600 dark:text-blue-400">
                  ...
                  <div className="text-right font-mono font-semibold text-emerald-600 dark:text-emerald-400">
                    {entry.solvedCount.toLocaleString()}
                  </div>
                  <div className="text-right font-mono text-sm font-semibold text-gray-500 dark:text-gray-400">
          ...
          <div className="text-right text-xs text-gray-500 dark:text-gray-400">
            XP 합계 {totalScore.toLocaleString()}
          </div>
        </div>
      </div>
    </div>
