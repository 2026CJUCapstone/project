...
export function Leaderboard() {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-[#121212] overflow-hidden">
      <div className="relative flex flex-col items-center justify-center py-8 bg-[#1e1e1e] border-b border-[#333] shrink-0">
        <div className="flex items-center gap-3 mb-5">
          <Trophy className="text-yellow-500" size={32} />
          <h1 className="text-2xl font-bold text-white tracking-wide">리더보드</h1>
        </div>
        
        <div className="flex bg-[#0d0d0d] p-1.5 rounded-lg border border-[#333]">
          <button
            onClick={() => setActiveTab('monthly')}
            className={`px-8 py-2 rounded-md text-base font-semibold transition-all ${
              activeTab === 'monthly' ? 'bg-[#2d2d2d] text-white shadow-sm' : 'text-gray-400 hover:text-gray-200'
            }`}
          >
            월간 랭킹
          </button>
          <button
            ...
            className={`px-8 py-2 rounded-md text-base font-semibold transition-all ${
              activeTab === 'weekly' ? 'bg-[#2d2d2d] text-white shadow-sm' : 'text-gray-400 hover:text-gray-200'
            }`}
          >
            주간 랭킹
          </button>
        ...
        <div className="absolute right-8 top-1/2 -translate-y-1/2 flex items-center gap-4 bg-[#252525] px-6 py-3 rounded-xl border border-[#333] shadow-sm">
          <img src={myRankData.avatar} alt="My Avatar" className="w-12 h-12 rounded-full bg-[#1e1e1e] border-2 border-[#444]" />
          <div className="flex flex-col">
            <span className="text-base font-bold text-gray-100">{myRankData.username}</span>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-sm font-bold text-yellow-500">#{myRankData.rank}</span>
              <span className="w-1.5 h-1.5 rounded-full bg-[#555]"></span>
              <span className="text-sm text-blue-400 font-mono font-bold">{myRankData.score.toLocaleString()} 점</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden p-6 relative">
        <div className="max-w-4xl mx-auto space-y-2 pb-10">
          <div className="grid grid-cols-[80px_1fr_120px] px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider border-b border-[#333] mb-4">
          ...
          {data.map((user) => (
            <div 
              ...
              className={`grid grid-cols-[80px_1fr_120px] items-center px-6 py-3 rounded-xl border transition-all ${
                user.rank <= 3 
                  ? 'bg-[#1a1a1a] border-[#333] hover:border-blue-500/50 hover:bg-[#252525]' 
            ...
            >
              ...
              <div className="flex items-center gap-4">
                <img src={user.avatar} alt="avatar" className="w-10 h-10 rounded-full bg-[#2d2d2d]" />
                <span className={`font-semibold ${user.rank <= 3 ? 'text-white text-lg' : 'text-gray-300'}`}>
                  {user.username}
                </span>
              </div>
